...
export default class extends Controller {
  ...
  connect() {
    ...
    this.loadCharacter(this.episodeValue);
  }

  async loadCharacter(episodeId) {
    this.loading.show();

    try {
      const response = await api.get(`/episode/${episodeId}`);
      console.log('details',response);
      this.renderEpisode(response.episode);
    ...
    this.detailsTarget.innerHTML = `
          <div class="bg-white rounded-xl shadow-md p-6 w-full  flex flex-col justify-between h-full">
            <div class="space-y-2">
                ...
                <p class="text-gray-500">Air Date: <span class="text-gray-700 font-medium">${episode.air_date}</span></p>
                <p class="text-gray-500">Episode Code: <span class="text-gray-700 font-medium">${episode.episode}</span></p>
            </div>
        </div>
        `;
